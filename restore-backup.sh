...
#   - Mode 1: Full restore (entire snapshot into restore dir)
#   - Mode 2: WordPress + Email + MySQL dumps + mail/LSWS configs for selected sites
#
# Reads:
...
KEEP_RESTORES=3   # how many restore dirs per archive prefix to keep under base
...
# -------------------------------------------------------------
# MODE 2: WORDPRESS + EMAIL + MYSQL DUMPS + MAIL/LSWS CONFIGS
# -------------------------------------------------------------
...
if ! borg list "$REPOSITORY::$ARCHIVE" --format '{path}{NEWLINE}' > "$TMP_PATHS"; then
  rm -f "$TMP_PATHS"
